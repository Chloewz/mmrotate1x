...
def parse_args():
    ...
    parser.add_argument("config", default='/home/odysseus/pyFiles/mmrotate1x/configs/sodaa-benchmarks/rotated-retinanet-rbox-le90_r50_fpn_1x_dota.py',help="test config file path")
    parser.add_argument(
        "prediction_path", default='/mnt/d/exp/sodaa_sob/a6000result/1107_retinanet/test/similiar.pkl',help="prediction path where test .pkl result"
    )
    parser.add_argument(
        "save_dir", default='/mnt/d/exp/sodaa_sob/a6000result/1107_retinanet/test',help="directory where confusion matrix will be saved"
    )
...
def calculate_confusion_matrix(
    ...
    confusion_matrix = np.zeros(shape=[num_classes, num_classes])   # 排除背景类
...
def analyze_per_img_dets(
    ...
    # unique_label = unique_label[unique_label != len(gt_labels)]

    for det_label in unique_label:
        # if det_label == len(gt_labels):
        #     continue
        ...
        for i, score in enumerate(det_scores):
            ...
            if score >= score_thr:
                ...
                if det_match == 0:  # BG FP
                    ...
                    confusion_matrix[-1, det_label] += 0
    for num_tp, gt_label in zip(true_positives, gt_labels):
        if num_tp == 0:  # FN
            # confusion_matrix[gt_label, -1] += 1
            confusion_matrix[gt_label, -1] += 0
