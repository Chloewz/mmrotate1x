...
class DETRHead(BaseModule):
    ...
    def loss_by_feat_single(
        ...
        # * new_tensor: 将源张量中的数据复制到目标张量(数据不共享), 同时提供了更细致的属性控制
    ...
    def _get_targets_single(
        ...
        # factor的作用是将归一化的坐标([0,1])还原为图片中的实际像素坐标，形状为[1,4]，每列对应img_w, img_h, img_w, img_h
        num_bboxes = bbox_pred.size(0)
        # convert bbox_pred from xywh, normalized to xyxy, unnormalized
        bbox_pred = bbox_cxcywh_to_xyxy(bbox_pred)
        bbox_pred = bbox_pred * factor  # 归一化坐标映射回实际像素坐标
        ...
        # * torch.nonzero(): 提取布尔Tensor中值为True的索引；输出是一个二维的Tensor，形状是[k,1]，k是正样本的数量
        neg_inds = (
            torch.nonzero(assign_result.gt_inds == 0, as_tuple=False)
            .squeeze(-1)    # * 去掉最后一个维度，结果从二维变为一维
            .unique()   # * 去重操作，确保索引唯一，此处通常不会有重复，但防止潜在错误
        )
        pos_assigned_gt_inds = assign_result.gt_inds[pos_inds] - 1
        pos_gt_bboxes = gt_bboxes[pos_assigned_gt_inds.long(), :]

        # label targets
        labels = gt_bboxes.new_full((num_bboxes,), self.num_classes, dtype=torch.long)  # 初始化为背景类，通常用#classes代替
        # * torch中的new_full: new_full(size, fill_value, dtype=None, device=None, requires_grad=False)
        # *     size: 尺寸；fill_value: 填充输出张良的数字
    ...
    def _predict_by_feat_single(
        ...
        if self.loss_cls.use_sigmoid:
            ...
            scores, indexes = cls_score.view(-1).topk(max_per_img)  # indexes=query_idx*num_classes+cls_idx
            det_labels = indexes % self.num_classes # 提取类别信息，因为展平索引的类别部分存储在低位，即提取cls_idx
            bbox_index = indexes // self.num_classes    # 提取查询信息，因为展平索引的查询部分存储在高位，即提取query_idx
