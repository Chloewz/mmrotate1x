...
class SODAAMetric(BaseMetric):
    """SODAA evaluation metric.
    ...
    default_prefix: Optional[str] = 'sodaa'

    def __init__(self,
                 iou_thrs: Union[float, List[float]] = [0.5,0.75,0.95],
    ...
    def compute_metrics(self, results: list) -> dict:
        ...
        # evaluation
        iou_thr_sodaa = None
        if iou_thr_sodaa is None:
            iou_thrs = np.linspace(
                0.5, 0.95, int(np.round((0.95-0.5)/0.05))+1, endpoint=True)

        SODAAEval = SODAAeval(gts, dets, numCats=len(dataset_name), nproc=nproc)
